...
from src.domain.entities.bairro import BairroEducacaoStats, BairroResumo
...
from .mongo_territorial_aggregation_repository import (
...
class MongoBairroRepository(MongoTerritorialAggregationRepository, IBairroRepository):
    ...
    async def get_resumo(self, bairro_id: str) -> BairroResumo | None:
        query = {"$or": [{"cd_bairro": bairro_id}, {"cd_bairro_ibge": bairro_id}]}
        ...
        if fallback_docs:
            return self._map_to_resumo(
                fallback_docs[0], source="setor_indicadores", oficial=False
            )
    ...
    def _map_to_resumo(self, doc: dict, source: str, oficial: bool) -> BairroResumo:
        ...
        return BairroResumo(
            ...
            socioeconomico=SocioeconomicoStats.model_validate(

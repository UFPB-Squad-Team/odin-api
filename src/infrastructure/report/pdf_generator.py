...
def _build_styles() -> dict:
    return {
        "CoverTitle": ParagraphStyle("CoverTitle", fontName="Helvetica-Bold", fontSize=30,
                                      leading=36, textColor=colors.white, alignment=TA_CENTER, spaceAfter=12),
        "CoverEntity": ParagraphStyle("CoverEntity", fontName="Helvetica-Bold", fontSize=22,
                                       leading=28, textColor=colors.HexColor(ODIN_GREEN), alignment=TA_CENTER, spaceAfter=8),
        "CoverSubtitle": ParagraphStyle("CoverSubtitle", fontName="Helvetica-Oblique", fontSize=12,
                                         leading=17, textColor=colors.HexColor(ODIN_WHITE_MUTED), alignment=TA_CENTER),
        "CoverDate": ParagraphStyle("CoverDate", fontName="Helvetica", fontSize=10,
                                     leading=13, textColor=colors.HexColor(ODIN_WHITE_MUTED), alignment=TA_CENTER),
        "SectionTitle": ParagraphStyle("SectionTitle", fontName="Helvetica-Bold", fontSize=13,
                                        leading=17, textColor=colors.white),
        "SubSectionTitle": ParagraphStyle("SubSectionTitle", fontName="Helvetica-Bold", fontSize=11,
                                           leading=15, textColor=colors.HexColor(ODIN_DARK_BLUE),
                                           spaceBefore=10, spaceAfter=5),
        "BodyText": ParagraphStyle("BodyText", fontName="Helvetica", fontSize=9.5, leading=14,
                                    textColor=colors.HexColor(ODIN_TEXT), alignment=TA_JUSTIFY,
                                    spaceBefore=2, spaceAfter=5),
        "BodyBold": ParagraphStyle("BodyBold", fontName="Helvetica-Bold", fontSize=9.5, leading=14,
                                    textColor=colors.HexColor(ODIN_TEXT)),
        "ValueText": ParagraphStyle("ValueText", fontName="Helvetica-Bold", fontSize=16, leading=20,
                                     textColor=colors.HexColor(ODIN_DARK_BLUE), alignment=TA_CENTER),
        "LabelText": ParagraphStyle("LabelText", fontName="Helvetica", fontSize=8, leading=11,
                                     textColor=colors.HexColor(ODIN_MUTED), alignment=TA_CENTER),
        "CaptionText": ParagraphStyle("CaptionText", fontName="Helvetica-Oblique", fontSize=8,
                                       leading=10, textColor=colors.HexColor(ODIN_MUTED),
                                       alignment=TA_CENTER, spaceBefore=2, spaceAfter=6),
        "SmallMuted": ParagraphStyle("SmallMuted", fontName="Helvetica", fontSize=8, leading=10,
                                      textColor=colors.HexColor(ODIN_MUTED)),
        "AnalysisBox": ParagraphStyle("AnalysisBox", fontName="Helvetica", fontSize=9, leading=13,
                                       textColor=colors.HexColor(ODIN_DARK_BLUE), alignment=TA_JUSTIFY,
                                       spaceBefore=0, spaceAfter=0, leftIndent=8, rightIndent=8),
        "BulletText": ParagraphStyle("BulletText", fontName="Helvetica", fontSize=9, leading=13,
                                      textColor=colors.HexColor(ODIN_TEXT), leftIndent=12,
...
def _header_footer(canvas, doc):
    ...
    if doc.page == 1:
        ...
        for i, (color, alpha) in enumerate([(ODIN_BLUE, 0.25), (ODIN_TEAL, 0.15), (ODIN_GREEN, 0.1)]):
            r = (8 - i * 2) * cm
        ...
        canvas.drawCentredString(PAGE_W / 2, 1.9 * cm,
    ...
    canvas.drawString(MARGIN_LEFT, PAGE_H - 0.6 * cm,
    ...
    canvas.setFont("Helvetica", 8)
    canvas.drawString(MARGIN_LEFT, 0.28 * cm,
                      "ODIN — Observatório de Dados Integrados do Nordeste | LEMA/UFPB")
...
def _section_header(title: str, icon: str = "●") -> Table:
    s = _build_styles()
    t = Table([[Paragraph(f"{icon}  {title}", s["SectionTitle"])]], colWidths=[CONTENT_W])
...
def _standard_table(header: list, rows: list, col_widths: list, alt_rows: bool = True) -> Table:
    ...
    if alt_rows:
        for i in range(1, len(all_data)):
            if i % 2 == 0:
                style.append(("BACKGROUND", (0, i), (-1, i), colors.HexColor(ODIN_LIGHT_GRAY)))
...
def _pct_colored(val: float | None, threshold_good=80, threshold_warn=60) -> str:
    if val is None:
        return "<font color='#6C757D'>—</font>"
    c = ODIN_GREEN if val >= threshold_good else (ODIN_ORANGE if val >= threshold_warn else ODIN_RED)
...
class DossierPDF:
    ...
    def _has_any_data(self) -> bool:
        d = self.data
        return (
            d.total_escolas > 0 or d.total_alunos > 0 or
            d.ideb_por_etapa or d.aprovacao_por_etapa or
            d.infraestrutura or d.escolas_por_dependencia or
            d.saneamento or d.habitacao or d.estrutura_etaria or d.raca or d.genero
        )
    ...
    def _generate_charts(self):
        ...
        if d.adequacao_docente_por_etapa or d.docentes_superior_por_etapa or d.alunos_por_turma_etapa:
    ...
    def _chart_ideb_grouped(self) -> bytes:
        ...
        bars2 = ax.bar(x + w / 2, aprov_vals, w, label="Aprovação/10", color=ODIN_TEAL, alpha=0.85, zorder=3)
        ax.axhline(meta_nacional, color=ODIN_RED, linewidth=1.4, linestyle="--", label=f"Meta Nacional ({meta_nacional})")

        for bar, val in zip(bars1, ideb_vals):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.05,
                    f"{val:.2f}", ha="center", va="bottom", fontsize=8, fontweight="bold", color=ODIN_BLUE)
        for bar, val in zip(bars2, aprov_vals):
            real_val = val * 10
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.05,
                    f"{real_val:.1f}%", ha="center", va="bottom", fontsize=8, color=ODIN_TEAL)
    ...
    def _chart_abandono_distorcao(self) -> bytes:
        ...
        b1 = ax.bar(x - w / 2, aband, w, label="Abandono (%)", color=ODIN_RED, alpha=0.85, zorder=3)
        b2 = ax.bar(x + w / 2, distor, w, label="Distorção Idade-Série (%)", color=ODIN_ORANGE, alpha=0.85, zorder=3)

        for bar, val in zip(b1, aband):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.2,
                    f"{val:.1f}%", ha="center", va="bottom", fontsize=8, fontweight="bold", color=ODIN_RED)
        for bar, val in zip(b2, distor):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.2,
                    f"{val:.1f}%", ha="center", va="bottom", fontsize=8, color=ODIN_ORANGE)
    ...
    def _chart_infraestrutura(self) -> bytes:
        ...
        bar_colors = [ODIN_GREEN if v >= 70 else (ODIN_ORANGE if v >= 40 else ODIN_RED) for v in values]

        fig, ax = plt.subplots(figsize=(10, 6.5))
        y = np.arange(len(labels))
        bars = ax.barh(y, values, color=bar_colors, edgecolor="white", linewidth=0.5, zorder=3)
    ...
    def _chart_dependencia_zona(self) -> bytes:
        ...
        if dep:
            ...
            wedges, texts, autotexts = ax1.pie(
                dep_vals, labels=dep_labels, colors=dep_colors, autopct="%1.1f%%",
            ...
            ax1.set_title(f"Dependência Administrativa\n({total_dep} escolas)", fontsize=10)
            for i, (val, label) in enumerate(zip(dep_vals, dep_labels)):
                ax1.annotate(f"{val} escolas", xy=(0, -1.3 - i * 0.18), ha="center", fontsize=8, color=dep_colors[i])
        else:
            ax1.set_title("Dependência Administrativa\n(sem dados)", fontsize=10)
            ax1.text(0, 0, "Não disponível", ha="center", va="center", fontsize=10, color=ODIN_MUTED)
            ax1.set_aspect(1)

        if zona:
            zona_labels = list(zona.keys())
            zona_vals = list(zona.values())
            zona_colors = [ODIN_TEAL if z == "Urbana" else ODIN_ORANGE for z in zona_labels]
            ax2.bar(zona_labels, zona_vals, color=zona_colors, width=0.4, zorder=3)
            for i, val in enumerate(zona_vals):
                ax2.text(i, val + 5, f"{val} ({val/sum(zona_vals)*100:.1f}%)",
            ...
            ax2.text(0.5, 0.5, "Não disponível", ha="center", va="center", fontsize=10, color=ODIN_MUTED)
            ax2.set_ylim(0, 1)
            ax2.set_yticks([])
    ...
    def _chart_top_schools(self) -> bytes:
        ...
        bar_colors = [ODIN_GOLD if v >= 6.5 else (ODIN_GREEN if v >= 6.0 else ODIN_TEAL) for v in idebs]

        fig, ax = plt.subplots(figsize=(10, 5.5))
        y = np.arange(len(names))
        bars = ax.barh(y, idebs, color=bar_colors, edgecolor="white", linewidth=0.5, zorder=3)

        for bar, val in zip(bars, idebs):
        ...
        if media_ideb is not None:
            ax.axvline(media_ideb, color=ODIN_BLUE, linestyle="--",
                       linewidth=1.2, label=f"Média municipal ({media_ideb:.2f})")
        ax.axvline(6.0, color=ODIN_RED, linestyle=":", linewidth=1.0, label="Meta nacional (6.0)")
        ...
        extra = []
        if media_ideb is not None:
            extra.append(plt.Line2D([0], [0], color=ODIN_BLUE, linestyle="--", label=f"Média municipal ({media_ideb:.2f})"))
        extra.append(plt.Line2D([0], [0], color=ODIN_RED, linestyle=":", label="Meta nacional (6.0)"))
    ...
    def _chart_estrutura_etaria(self) -> bytes:
        ...
        if etaria:
            ...
            wedges, texts, autotexts = ax1.pie(
                vals_e, labels=labels_e, colors=colors_e, autopct="%1.1f%%",
        ...
        if raca:
            ...
            bars = ax2.bar(labels_r, vals_r, color=colors_r, edgecolor="white", linewidth=0.5, zorder=3)
    ...
    def _chart_saneamento_habitacao(self) -> bytes:
        ...
        if san:
            ...
            ax1.text(0.5, 0.5, "Não disponível", ha="center", va="center", fontsize=10, color=ODIN_MUTED)
            ax1.set_ylim(0, 1)
            ax1.set_yticks([])
    ...
    def _chart_docentes(self) -> bytes:
        ...
        b1 = ax.bar(x - w, doc_sup, w, label="Docentes c/ Ensino Superior (%)", color=ODIN_BLUE, zorder=3)
        b2 = ax.bar(x, afd, w, label="Adequação Funcional Docente (%)", color=ODIN_TEAL, zorder=3)

        for bar, val in zip(b1, doc_sup):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.3,
                    f"{val:.1f}", ha="center", va="bottom", fontsize=7.5, fontweight="bold", color=ODIN_BLUE)
        for bar, val in zip(b2, afd):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.3,
                    f"{val:.1f}", ha="center", va="bottom", fontsize=7.5, color=ODIN_TEAL)

        ax2 = ax.twinx()
        ax2.plot(x + w / 2, alunos_turma, "o--", color=ODIN_ORANGE, linewidth=2,
                 markersize=7, label="Alunos por Turma (eixo dir.)")
        for i, (xi, val) in enumerate(zip(x, alunos_turma)):
            ax2.text(xi + w / 2, val + 0.3, f"{val:.1f}", ha="center", va="bottom",
                     fontsize=8, color=ODIN_ORANGE, fontweight="bold")
        if alunos_turma:
    ...
    def _build_cover(self) -> None:
        ...
        self._story.extend([
            ...
            Spacer(1, 8),
            Paragraph(f"Gerado em {_data_por_extenso(datetime.now())}", s["CoverDate"]),
            Spacer(1, 2.5 * cm),
            Paragraph("Indicadores Educacionais e Socioeconômicos", s["CoverSubtitle"]),
            Spacer(1, 0.6 * cm),
            Paragraph("Censo Escolar INEP/MEC · Censo Demográfico IBGE", s["CoverDate"]),
            PageBreak(),
        ])
    ...
    def _build_summary(self) -> None:
        ...
        pop_text = f"{d.populacao_total:,}".replace(",", ".") if d.populacao_total else "dados não disponíveis"
        ...
        kpis = [
            ...
            _kpi_card("Total de Matrículas", f"{d.total_alunos:,}".replace(",", "."), ODIN_TEAL),
        ...
        if d.ideb_por_etapa:
            for etapa, val in d.ideb_por_etapa.items():
                meta = "6,0" if "Iniciais" in etapa else "5,5" if "Finais" in etapa else "5,2"
                color = ODIN_GREEN if val >= 6.0 else ODIN_ORANGE if val >= 4.5 else ODIN_RED
        ...
        if d.ideb_por_etapa and "Anos Iniciais" in d.ideb_por_etapa:
            analysis_parts.append(f"IDEB nos Anos Iniciais é {d.ideb_por_etapa['Anos Iniciais']:.2f}")
        if d.aprovacao_por_etapa and "Anos Iniciais" in d.aprovacao_por_etapa:
            analysis_parts.append(f"aprovação de {d.aprovacao_por_etapa['Anos Iniciais']:.1f}% nos Anos Iniciais")
        if d.aprovacao_por_etapa and "Ensino Médio" in d.aprovacao_por_etapa:
            analysis_parts.append(f"{d.aprovacao_por_etapa['Ensino Médio']:.1f}% no Ensino Médio")
        if analysis_parts:
            analysis_text = "📌 <b>Análise geral:</b> " + "; ".join(analysis_parts) + "."
        ...
        for e in etapas:
            ...
            row.append(Paragraph(_pct_colored(val, 80, 60) if val is not None else "<font color='#6C757D'>—</font>", s["BodyText"]))
        rows_res.append(row)
        # Abandono
        ...
        for e in etapas:
            ...
            row.append(Paragraph(f"{val:.1f}" if val is not None else "—", s["BodyText"]))
        rows_res.append(row)
        # Docentes c/ superior
        row = [Paragraph("<b>Docentes c/ Superior (%)</b>", s["BodyText"])]
        for e in etapas:
            val = d.docentes_superior_por_etapa.get(e)
            row.append(Paragraph(_pct_colored(val, 90, 70) if val is not None else "<font color='#6C757D'>—</font>", s["BodyText"]))
        rows_res.append(row)

        self._story.append(_standard_table(header_res, rows_res, [5.5*cm, 3.5*cm, 3.5*cm, 3.5*cm]))
    ...
    def _build_educational(self) -> None:
        ...
        self._story.append(_section_header("Indicadores Educacionais — Desempenho", "📊"))
        self._story.append(Spacer(1, 10))
        ...
        if "ideb_grouped" in self._charts:
            ...
            self._story.append(Paragraph("IDEB e Taxa de Aprovação por etapa. Linha tracejada = meta nacional.", s["CaptionText"]))
        self._story.append(Spacer(1, 8))
        if d.abandono_por_etapa or d.distorcao_idade_serie_por_etapa:
            self._story.append(Paragraph("Abandono Escolar e Distorção Idade-Série", s["SubSectionTitle"]))
            self._story.append(Paragraph(
            ...
            if "abandono_distorcao" in self._charts:
                self._story.append(_chart_image(self._charts["abandono_distorcao"], 15.5, 4.8))
                self._story.append(Paragraph("Abandono escolar e distorção idade-série por etapa.", s["CaptionText"]))
        self._story.append(PageBreak())

        self._story.append(_section_header("Corpo Docente e Condições de Ensino", "👩‍🏫"))
        self._story.append(Spacer(1, 10))
        self._story.append(Paragraph(
        ...
        if "docentes" in self._charts:
            ...
            self._story.append(Paragraph("Qualificação docente (%) e número médio de alunos por turma.", s["CaptionText"]))
        self._story.append(Spacer(1, 10))

        header_doc = ["Etapa", "Docentes c/ Superior", "Adequação Docente (AFD)", "Horas-Aula/Dia", "Alunos/Turma"]
        rows_doc = []
        etapas = ["Anos Iniciais", "Anos Finais", "Ensino Médio"]
        for e in etapas:
            ...
            rows_doc.append([
                ...
                Paragraph(_pct_colored(sup, 90, 70) if sup is not None else "<font color='#6C757D'>—</font>", s["BodyText"]),
                Paragraph(_pct_colored(afd, 85, 70) if afd is not None else "<font color='#6C757D'>—</font>", s["BodyText"]),
                Paragraph(f"{horas:.1f}h" if horas is not None else "—", s["BodyText"]),
                Paragraph(f"{alunos:.1f}" if alunos is not None else "—", s["BodyText"]),
            ])
        self._story.append(_standard_table(header_doc, rows_doc, [4.5*cm, 3.5*cm, 3.8*cm, 2.7*cm, 2.5*cm]))
        self._story.append(Spacer(1, 10))
    ...
    def _build_infrastructure(self) -> None:
        ...
        if "infra_bars" in self._charts:
            ...
            self._story.append(Paragraph("Cobertura de infraestrutura. Verde ≥ 70% (adequado), laranja 40–69% (atenção), vermelho < 40% (crítico).", s["CaptionText"]))
        self._story.append(Spacer(1, 10))

        header_inf = ["Item de Infraestrutura", "Cobertura", "Status", "Nº Estimado de Escolas"]
        rows_inf = []
        infra_sorted = sorted(d.infraestrutura.items(), key=lambda x: x[1], reverse=True)
        for label, val in infra_sorted:
            status = "✔ Adequado" if val >= 70 else ("⚠ Atenção" if val >= 40 else "✗ Crítico")
            status_color = ODIN_GREEN if val >= 70 else (ODIN_ORANGE if val >= 40 else ODIN_RED)
            n_escolas = int(d.total_escolas * val / 100) if d.total_escolas else 0
            rows_inf.append([
                Paragraph(f"<b>{label}</b>", s["BodyText"]),
                Paragraph(f'<font color="{status_color}"><b>{val:.1f}%</b></font>', s["BodyText"]),
                Paragraph(f'<font color="{status_color}"><b>{status}</b></font>', s["BodyText"]),
                Paragraph(f"≈ {n_escolas} de {d.total_escolas}", s["BodyText"]),
            ])
        self._story.append(_standard_table(header_inf, rows_inf, [6.0*cm, 2.8*cm, 3.0*cm, 4.2*cm]))
        self._story.append(PageBreak())
    ...
    def _build_distribution(self) -> None:
        ...
        if "dependencia_zona" in self._charts:
            self._story.append(_chart_image(self._charts["dependencia_zona"], 15.5, 5.2))
            self._story.append(Paragraph("Distribuição por dependência administrativa e localização.", s["CaptionText"]))
        self._story.append(Spacer(1, 10))
        if d.escolas_por_dependencia:
            header_dep = ["Rede", "Nº de Escolas", "Participação (%)", "Matrículas Estimadas"]
            ...
            for rede, qtd in d.escolas_por_dependencia.items():
                ...
                rows_dep.append([
                    ...
                    Paragraph(f'<font color="{ODIN_BLUE}"><b>{qtd}</b></font>', s["BodyText"]),
                    Paragraph(f"{pct:.1f}%", s["BodyText"]),
                    Paragraph(f"≈ {int(qtd * (d.total_alunos / total_dep if total_dep else 0)):,}".replace(",", "."), s["BodyText"]),
                ])
            # Removed total row because it may not match the actual total_escolas
            self._story.append(_standard_table(header_dep, rows_dep, [4.5*cm, 3.5*cm, 3.5*cm, 5.0*cm]))
        self._story.append(PageBreak())
    ...
    def _build_top_schools(self) -> None:
        ...
        if not d.top_escolas:
            return
        self._story.append(_section_header("Top Escolas por IDEB — Anos Iniciais", "🏆"))
        self._story.append(Spacer(1, 10))
        self._story.append(Paragraph(
        ...
        if "top_schools" in self._charts:
            ...
            self._story.append(Paragraph("Ranking das 10 escolas com maior IDEB (Anos Iniciais).", s["CaptionText"]))
        self._story.append(Spacer(1, 10))
        ...
        for i, school in enumerate(d.top_escolas[:10], 1):
            ...
            rows_esc.append([
                ...
                Paragraph(f'<font color="{ideb_color}"><b>{ideb_v:.1f}</b></font>', s["BodyText"]),
                Paragraph(school.get("dependencia", "-"), s["BodyText"]),
                Paragraph(school.get("localizacao", "-"), s["BodyText"]),
            ])
        self._story.append(_standard_table(header_esc, rows_esc, [2.0*cm, 6.5*cm, 2.0*cm, 2.8*cm, 2.7*cm]))
        self._story.append(PageBreak())

    def _build_socioeconomic(self) -> None:
        s = self.styles
        d = self.data
        has_demo = d.estrutura_etaria or d.raca or d.genero or d.populacao_total is not None
        ...
        if has_demo:
            ...
            if d.populacao_total:
                self._story.append(Paragraph(
                    f"População total: <b>{d.populacao_total:,}".replace(",", ".") + " habitantes</b> (IBGE).",
                    s["BodyText"],
                ))
            ...
            kpis_demo = []
            if d.taxa_analfabetismo is not None:
                kpis_demo.append(_kpi_card("Analfabetismo (15+)", f"{d.taxa_analfabetismo:.1f}%", ODIN_RED))
            if d.razao_dependencia is not None:
                kpis_demo.append(_kpi_card("Razão de Dependência", f"{d.razao_dependencia:.1f}", ODIN_ORANGE))
            if d.pct_responsavel_feminino is not None:
                kpis_demo.append(_kpi_card("Responsável feminino", f"{d.pct_responsavel_feminino:.1f}%", ODIN_PURPLE))
            if d.media_moradores_domicilio is not None:
                kpis_demo.append(_kpi_card("Moradores/Domicílio", f"{d.media_moradores_domicilio:.2f}", ODIN_TEAL))
            if kpis_demo:
                self._story.append(_kpi_row(kpis_demo))
                self._story.append(Spacer(1, 8))

            if "estrutura_etaria" in self._charts and (d.estrutura_etaria or d.raca):
                self._story.append(_chart_image(self._charts["estrutura_etaria"], 15.5, 5.2))
                self._story.append(Paragraph("Estrutura etária e composição racial.", s["CaptionText"]))
                self._story.append(Spacer(1, 8))

            header_demo = ["Indicador Demográfico", "Valor", "Referência"]
            rows_demo = []
            if d.populacao_total is not None:
                rows_demo.append([Paragraph("População total", s["BodyText"]), Paragraph(f"{d.populacao_total:,}".replace(",", "."), s["BodyText"]), Paragraph("IBGE", s["BodyText"])])
            if d.taxa_analfabetismo is not None:
                rows_demo.append([Paragraph("Taxa de analfabetismo (15+)", s["BodyText"]), Paragraph(f'{d.taxa_analfabetismo:.1f}%', s["BodyText"]), Paragraph("IBGE", s["BodyText"])])
            if d.razao_dependencia is not None:
                rows_demo.append([Paragraph("Razão de dependência", s["BodyText"]), Paragraph(f"{d.razao_dependencia:.1f}", s["BodyText"]), Paragraph("IBGE", s["BodyText"])])
            if d.pct_responsavel_feminino is not None:
                rows_demo.append([Paragraph("Responsável feminino", s["BodyText"]), Paragraph(f"{d.pct_responsavel_feminino:.1f}%", s["BodyText"]), Paragraph("IBGE", s["BodyText"])])
            if d.media_moradores_domicilio is not None:
                rows_demo.append([Paragraph("Média moradores/domicílio", s["BodyText"]), Paragraph(f"{d.media_moradores_domicilio:.2f}", s["BodyText"]), Paragraph("IBGE", s["BodyText"])])
            if d.estrutura_etaria:
                for key, val in d.estrutura_etaria.items():
                    rows_demo.append([Paragraph(key, s["BodyText"]), Paragraph(f"{val:.1f}%", s["BodyText"]), Paragraph("IBGE", s["BodyText"])])
            if d.raca:
                for key, val in d.raca.items():
                    rows_demo.append([Paragraph(f"Pop. {key.lower()}", s["BodyText"]), Paragraph(f"{val:.1f}%", s["BodyText"]), Paragraph("IBGE", s["BodyText"])])
            if d.genero:
                for key, val in d.genero.items():
                    rows_demo.append([Paragraph(f"Pop. {key.lower()}", s["BodyText"]), Paragraph(f"{val:.1f}%", s["BodyText"]), Paragraph("IBGE", s["BodyText"])])
            if rows_demo:
                self._story.append(_standard_table(header_demo, rows_demo, [7.5*cm, 4.5*cm, 4.0*cm]))
                self._story.append(Spacer(1, 10))

        if has_san:
            self._story.append(Paragraph("Saneamento e Habitação", s["SubSectionTitle"]))
            self._story.append(Paragraph(
            ...
            if "saneamento_habitacao" in self._charts:
                self._story.append(_chart_image(self._charts["saneamento_habitacao"], 15.5, 5.5))
                self._story.append(Paragraph("Saneamento básico e condições de habitação.", s["CaptionText"]))
                self._story.append(Spacer(1, 8))
            ...
            if d.saneamento:
                for key, val in d.saneamento.items():
                    ...
                    rows_san.append([
                        ...
                        Paragraph(f'<font color="{c}"><b>{val:.1f}%</b></font>', s["BodyText"]),
                        Paragraph("Adequado" if val >= 70 else "Atenção", s["BodyText"]),
                    ])
            if d.habitacao:
            ...
            if rows_san:
                self._story.append(_standard_table(header_san, rows_san, [6.5*cm, 3.5*cm, 6.0*cm]))
        self._story.append(PageBreak())

    def _build_footer(self) -> None:
        s = self.styles
        self._story.append(Spacer(1, 20))
        self._story.append(HRFlowable(width="100%", thickness=0.5, color=colors.HexColor(ODIN_MEDIUM_GRAY)))
        self._story.append(Spacer(1, 8))
    ...
    def render(self) -> io.BytesIO:
        ...
        self._story.extend([
            ...
            _kpi_row([
                ...
                _kpi_card("Total de Alunos", f"{d.total_alunos:,}".replace(",", "."), ODIN_GREEN),
            ]),
            Spacer(1, 10),
        ])
        if d.ideb_por_etapa:
            ...
            self._story.append(_standard_table(header, rows, [CONTENT_W/2, CONTENT_W/2]))
        self._story.append(PageBreak())
        self._story.append(Paragraph("Metodologia e Fontes", s["SubSectionTitle"]))
        self._story.append(Paragraph(
